...
<div class="container">
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination">
			<li class="page-item"><a class="page-link" href="<%if(pageIndex<=numOfPages && pageIndex>1){%>pageLibrarianBranch?pageNo=<%=pageIndex-1%><%} %>"
				aria-label="Previous"> <span aria-hidden="true">&laquo;</span> <span
					class="sr-only">Previous</span>
			</a></li>
			<%for(int i=1; i<=numOfPages; i++){ %>
			<li class="page-item"><a class="page-link" href="pageLibrarianBranch?pageNo=<%=i%>"><%=i%></a></li>
			<%} %>
			<li class="page-item"><a class="page-link" href="<%if(pageIndex<numOfPages){%>pageLibrarianBranch?pageNo=<%=pageIndex+1%><%} %>"
	...
	<table class="table table-striped">
		...
		<tr>
			...
			<td><button type="button"
					onclick="javascript:location.href='selectedBranch?branchId=<%=a.getBranchId()%>'"
